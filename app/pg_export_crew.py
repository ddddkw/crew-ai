...
from my_tools import TOOL_CLASSES, get_tool_class_key, resolve_tool_class
...
class PageExportCrew:
    ...
    def generate_streamlit_app(self, crew, output_dir):
        ...
        custom_tools_used = any(get_tool_class_key(tool) in ["CustomApiTool", "CustomFileWriteTool", "CustomCodeInterpreterTool", "ScrapeWebsiteToolEnhanced", "CSVSearchToolEnhanced"] 
        ...
        def format_tool_instance(tool):
            tool_key = get_tool_class_key(tool)
            tool_class = TOOL_CLASSES.get(tool_key)
            if tool_class:
                params = ', '.join([f'{key}={json_dumps_python(value)}' for key, value in tool.parameters.items() if value is not None])
                return f'{tool_key}({params})' if params else f'{tool_key}()'
            return None
...
Agent(
    ...
    tools=[{', '.join([tool_text for tool_text in (format_tool_instance(tool) for tool in agent.tools) if tool_text])}],
...
"""
    ...
    def import_crew_from_json(self, crew_data):
        ...
        for tool_data in crew_data['tools']:
            tool_class = resolve_tool_class(tool_data.get('name'))
            if tool_class is None:
                raise ValueError(f"Unknown tool type: {tool_data.get('name')}")
    ...
    def draw(self):
        draw_page_header(
            ...
            subtitle=t("export.title"),
            count=len(ss.crews) if 'crews' in ss else None,
        )

        with st.container(border=True, key="export-workspace-card"):
