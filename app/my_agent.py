...
class MyAgent:
    def __init__(self, id=None, role=None, backstory=None, goal=None, temperature=None, allow_delegation=False, verbose=False, cache= None, llm_provider_model=None, max_iter=None, created_at=None, tools=None, knowledge_source_ids=None):
        ...
        self.llm_provider_model = available_models[0] if llm_provider_model is None and available_models else llm_provider_model
    ...
    def get_crewai_agent(self) -> Agent:
        if not self.llm_provider_model:
            raise ValueError(t('agent.no_llm_configured'))
    ...
    def is_valid(self, show_warning=False):
        if not self.llm_provider_model or self.llm_provider_model not in llm_providers_and_models():
            if show_warning:
    ...
    def validate_llm_provider_model(self):
        ...
        elif self.llm_provider_model not in available_models:
            self.llm_provider_model = available_models[0]

    def draw(self, key=None):
        self.validate_llm_provider_model()
        model_name = self.llm_provider_model.split(": ", 1)[1] if self.llm_provider_model and ": " in self.llm_provider_model else t('agent.no_llm_short')
        expander_title = f"{self.role[:60]} -{model_name}" if self.is_valid() else f"❗ {self.role[:20]} -{model_name}"
