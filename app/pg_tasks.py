...
class PageTasks:
    ...
    def draw(self):
        ...
        draw_page_header(
            ...
            subtitle=t("task.all_tasks"),
            count=len(ss.tasks),
        )

        with st.container(border=True, key="tasks-workspace-card"):
            ...
            tabs = [t("task.all_tasks")] + [t("task.unassigned_tasks")] + [crew.name for crew in ss.crews]
            ...
            if len(ss.tasks) == 0:
                st.write(t("task.none_defined"))
                st.button(t("button.create_task"), on_click=self.create_task, disabled=editing)
